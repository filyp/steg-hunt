...
good_token = 777
bad_token = 666
batch_size = 36 # training_args.per_device_train_batch_size
rand_size = batch_size // 12
...
# for i in range(1):
    ...
    # train bob and cid for a bit, on ann's new actions
    key_emsg = torch.cat((key, emsg), axis=1)
    key_emsg_msg = torch.cat((key_emsg, msg), axis=1)
    bob_trainer.train_dataset = DatasetWrapper(key_emsg_msg)
    emsg_msg = torch.cat((emsg, msg), axis=1)
    cid_trainer.train_dataset = DatasetWrapper(emsg_msg)
    for i in range(6):
        bob_trainer.train()
        cid_trainer.train()
    ...
    # train ann
    ann_trainer.train_dataset = DatasetWrapper(s_key_msg_emsg)
    for i in range(6):
        ann_trainer.train()
